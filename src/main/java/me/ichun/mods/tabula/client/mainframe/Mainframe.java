package me.ichun.mods.tabula.client.mainframe;
...
public class Mainframe
{
    ...
    public HashMap<String, Integer> listeners = new HashMap<>();
    public ArrayList<String> editors = new ArrayList<>();

    public boolean allowEditing;

    public ArrayList<ProjectInfo> projects = new ArrayList<>(); //each workspace tab should be a project.

    public HashMap<String, ArrayList<byte[]>> projectParts = new HashMap<>();
    ...
    public void tick()
    {
        ...
        while(ite.hasNext())
        {
            ...
            if(!e.getKey().equals(Minecraft.getMinecraft().getSession().getUsername()))
            {
                ...
                {
                    ite.remove();
                    sendChat("System", I18n.translateToLocalFormatted("system.timeout", e.getKey()));
                }
            }
        }
    }
    ...
    public void addListener(String id, boolean isEditor)
    {
        if(!listeners.containsKey(id))
        {
            ...
            if(!id.equals(Minecraft.getMinecraft().getSession().getUsername()))
            {
                sendChat("System", I18n.translateToLocalFormatted("system.joinedSession", id));
                for(ProjectInfo proj : projects)
                {
        ...
        if(isEditor && !editors.contains(id))
        {
            ...
            if(!id.equals(Minecraft.getMinecraft().getSession().getUsername()))
            {
                sendChat("System", I18n.translateToLocalFormatted("system.isEditor", id));
            }
        }
    ...
    public void removeListener(String id)
    {
        ...
        {
            listeners.remove(id);
            sendChat("System", I18n.translateToLocalFormatted("system.leftSession", id));
    ...
    public void addEditor(String id)
    {
        ...
        {
            editors.add(id);
            sendChat("System", I18n.translateToLocalFormatted("system.addEditor", Minecraft.getMinecraft().getSession().getUsername(), id));
            Tabula.channel.sendToServer(new PacketIsEditor(Minecraft.getMinecraft().getSession().getUsername(), id, true));

            String[] editors = Tabula.config.editors.split(", *");
            ArrayList<String> editorArray = new ArrayList<>();
        ...
        {
            ProjectHelper.receiveChat("System: " + I18n.translateToLocalFormatted("system.alreadyEditor", id));
        }
    }
    ...
    public void removeEditor(String id)
    {
        ...
        {
            editors.remove(id);
            sendChat("System", I18n.translateToLocalFormatted("system.removeEditor", Minecraft.getMinecraft().getSession().getUsername(), id));
        ...
        {
            ProjectHelper.receiveChat("System: " + I18n.translateToLocalFormatted("system.notEditor", id));
        }
    }
